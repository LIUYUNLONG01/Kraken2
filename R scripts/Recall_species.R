...
# Read data
df <- read.csv("recall-species-cs.csv")

# Convert data from wide to long format for ggplot2
df_long <- reshape2::melt(df, id.vars = "confidence", variable.name = "Database", value.name = "Recall")
...
# Plot boxplot
ggplot(df_long, aes(x = Database, y = Recall, fill = confidence)) +
