...
# Read data
data <- read.csv("composition_phylum.csv")  
...
phylum_abundance <- data %>%
  group_by(phylum) %>%
  summarize(total_abundance = sum(abundance)) %>%
  arrange(desc(total_abundance))

# Extract top 11 phyla
top_phyla <- phylum_abundance$phylum[1:11]

# Define colors
top_colors <- c("#831b52", "#386325", "#ffff99", "#b15928", "#ff7f00", "#e31a1c", "#a6cee3", "#33a02c", "#1f78b4", "#fb9a99", "black")
additional_phyla <- setdiff(unique(data$phylum), c(top_phyla, "others"))
additional_colors <- colorRampPalette(brewer.pal(9, "Set3"))(length(additional_phyla))

# Create color vector, set "others" to black
color_vector <- setNames(c(top_colors, additional_colors, "black"), c(top_phyla, additional_phyla, "others"))

# Plot stacked bar chart
ggplot(data, aes(x = cs, y = abundance, fill = phylum)) +
